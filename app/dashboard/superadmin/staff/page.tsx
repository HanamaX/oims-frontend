...
export default function StaffManagementPage() {
            ...
            if (branchesResponse.data && branchesResponse.data.data) {
              ...
              try {
                ...
                if (adminsResponse.data && adminsResponse.data.data) {
                  // Map API response to our StaffMember structure
  ...
  const confirmAdminUpdate = async () => {
    ...
    try {
      // Build the request payload according to the SuperAdminUpdateRequest interface
      // Only sending what superadmins are allowed to update
      // For ROLE_SUPER_ADMIN, branch assignment isn't needed as they have global access
      ...
      if (staffToUpdate.role !== "ROLE_SUPER_ADMIN") {
      ...
      if (existingStaff) {
        ...
        if (staffToUpdate.role !== "ROLE_SUPER_ADMIN" && staffToUpdate.branchPublicId) {
  ...
  return (
    <div className="space-y-6">
      ...
      {center && branches.length === 0 && (
        <Alert>
          ...
          <AlertDescription>
            ...
            <Button variant="link" className="p-0 h-auto" onClick={() => router.push("/dashboard/superadmin/branches")}>
              Create Branch
            </Button>
          </AlertDescription>
        </Alert>
      ...
      {center && branches.length > 0 && showForm && (
        <Card>
          <form onSubmit={handleSubmit}>
            <CardHeader>
              <CardTitle>Add New Staff Member</CardTitle>
              <CardDescription>Enter the details of the staff member</CardDescription>
            </CardHeader>
        ...
        <DialogContent className="bg-white">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-4">
              {staffToUpdate?.role !== "ROLE_SUPER_ADMIN" && (
                <div className="space-y-2">
              ...
              <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800">
                <p className="font-medium mb-1">Superadmin Permissions</p>
                {staffToUpdate?.role === "ROLE_SUPER_ADMIN" ? (
                  <p>Super Admins don't need branch assignments as they have access to all branches. You can only modify the account status for Super Admins.</p>
                ) : (
                  <p>You can only modify the branch assignment and account status for this admin. To change other details, ask the admin to update their own profile.</p>
                )}
                <p className="mt-1">Note: Suspending an admin will prevent them from logging in but preserve their account data.</p>
              </div>
            </div>
          </div>
          <DialogFooter>
